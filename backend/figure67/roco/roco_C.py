...
def kernel_0(
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        tl.store(C_exp_ptr + offset_7, tl.exp(tl.dot(Q, temp_8).to(tl.float16).to(tl.float32)).to(tl.float16), mask=mask_5)
        temp_9 = tl.permute(temp_7, (0, 2, 1))
        C_sum = ((1 * C_sum).to(tl.float16) + tl.sum(tl.exp(tl.dot(Q, temp_9).to(tl.float16).to(tl.float32)).to(tl.float16), axis=2, dtype=tl.float16)).to(tl.float16)
    # Sequential loop p from 0 to 1024 with tile size BLOCK_P
    for p in range(0, 1024, BLOCK_P):
        ...
        C_div = (temp_10 / C_sum[:, :, None]).to(tl.float16)
        ...
        O = (tl.dot(C_div, temp_11).to(tl.float16) + (1 * O).to(tl.float16)).to(tl.float16)
        offset_10 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_out1_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * C_out1_stride1
        mask_8 = (p_indices < P+M)[None, :]
        tl.store(C_out1_ptr + offset_10, tl.sum(C_div, axis=1), mask=mask_8)
        offset_11 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_out2_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * C_out2_stride1
        mask_9 = (p_indices < P+M)[None, :]
        tl.store(C_out2_ptr + offset_11, tl.sum((C_div * C_div).to(tl.float16), axis=1, dtype=tl.float16), mask=mask_9)
    ...
    tl.store(O2_ptr + offset_12, tl.reshape(temp_12, (M, D)), mask=mask_10)
