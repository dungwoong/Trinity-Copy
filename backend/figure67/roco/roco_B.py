...
def kernel_0(
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        C_sum = (tl.sum(tl.exp(tl.dot(Q, temp_9).to(tl.float16).to(tl.float32)).to(tl.float16), axis=2, dtype=tl.float16) + (1 * C_sum).to(tl.float16)).to(tl.float16)
    # Sequential loop p from 0 to 1024 with tile size BLOCK_P
    for p in range(0, 1024, BLOCK_P):
        ...
        tl.store(C_out1_ptr + offset_9, tl.sum(C_div, axis=1), mask=mask_7)
        offset_10 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_out2_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * C_out2_stride1
        mask_8 = (p_indices < P+M)[None, :]
        tl.store(C_out2_ptr + offset_10, tl.sum((C_div * C_div).to(tl.float16), axis=1, dtype=tl.float16), mask=mask_8)
        offset_11 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * V_cache_stride0 + (p + tl.arange(0, BLOCK_P))[None, :, None] * V_cache_stride1 + (tl.arange(0, 128))[None, None, :] * V_cache_stride2
        mask_9 = (p_indices < P+M)[None, :, None]
        temp_11 = tl.load(V_cache_ptr + offset_11, mask=mask_9, other=0.0)
        O = ((O * 1).to(tl.float16) + tl.dot(temp_10, temp_11).to(tl.float16)).to(tl.float16)
