...
def kernel_0(
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        C = tl.dot(Q, temp_5).to(tl.float16)
        ...
        temp_6 = tl.load(noise_ptr + offset_7, mask=mask_5, other=0.0)
        temp_7 = tl.permute(temp_4, (0, 2, 1))
        C_perturb = ((temp_6 + tl.dot(Q, temp_7).to(tl.float16)).to(tl.float16) / 1.5).to(tl.float16)
        C_exp = tl.exp(C.to(tl.float32)).to(tl.float16)
        ...
        tl.store(C_exp_perturb_ptr + offset_8, tl.exp(C_perturb.to(tl.float32)).to(tl.float16), mask=mask_6)
        C_sum = (tl.sum(tl.exp(C.to(tl.float32)).to(tl.float16), axis=2, dtype=tl.float16) + (1 * C_sum).to(tl.float16)).to(tl.float16)
        C_sum_perturb = (tl.sum(tl.exp(C_perturb.to(tl.float32)).to(tl.float16), axis=2, dtype=tl.float16) + (1 * C_sum_perturb).to(tl.float16)).to(tl.float16)
        offset_9 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * V_cache_stride0 + (p + tl.arange(0, BLOCK_P))[None, :, None] * V_cache_stride1 + (tl.arange(0, 128))[None, None, :] * V_cache_stride2
        mask_7 = (p_indices < P+M)[None, :, None]
        temp_8 = tl.load(V_cache_ptr + offset_9, mask=mask_7, other=0.0)
        O = ((1 * O).to(tl.float16) + tl.dot(C_exp, temp_8).to(tl.float16)).to(tl.float16)
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        temp_9 = tl.load(C_exp_perturb_ptr + offset_10, mask=mask_8, other=0.0)
        offset_11 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_out_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * C_out_stride1
        mask_9 = (p_indices < P+M)[None, :]
        tl.store(C_out_ptr + offset_11, tl.sum((temp_9 / C_sum_perturb[:, :, None]).to(tl.float16), axis=1, dtype=tl.float16), mask=mask_9)
    ...
    tl.store(O2_ptr + offset_12, tl.reshape(temp_10, (M, D)), mask=mask_10)
