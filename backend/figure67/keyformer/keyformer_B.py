...
def kernel_0(
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        O = ((O * 1).to(tl.float16) + tl.dot(C_exp, temp_11).to(tl.float16)).to(tl.float16)
    # Sequential loop p from 0 to 1024 with tile size BLOCK_P
    for p in range(0, 1024, BLOCK_P):
        ...
        tl.store(C_out_ptr + offset_11, tl.sum((temp_12 / C_sum_perturb[:, :, None]).to(tl.float16), axis=1, dtype=tl.float16), mask=mask_9)
