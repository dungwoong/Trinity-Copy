...
def kernel_0(
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        offset_17 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * V_cache_stride0 + (p + tl.arange(0, BLOCK_P))[None, :, None] * V_cache_stride1 + (tl.arange(0, 128))[None, None, :] * V_cache_stride2
        mask_15 = (p_indices < P+M)[None, :, None]
        temp_15 = tl.load(V_cache_ptr + offset_17, mask=mask_15, other=0.0)
        O = ((1 * O).to(tl.float16) + tl.dot(temp_14, temp_15).to(tl.float16)).to(tl.float16)
