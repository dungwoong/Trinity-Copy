...
def kernel_0(
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        C_sum = ((1 * C_sum).to(tl.float16) + tl.sum(tl.exp(C.to(tl.float32)).to(tl.float16), axis=2, dtype=tl.float16)).to(tl.float16)
        ...
        O = ((1 * O).to(tl.float16) + tl.dot(C_exp, temp_11).to(tl.float16)).to(tl.float16)
