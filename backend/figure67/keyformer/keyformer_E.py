...
def kernel_0(
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        C_sum = ((C_sum * 1).to(tl.float16) + tl.sum(temp_11, axis=2)).to(tl.float16)
        ...
        C_sum_perturb = ((C_sum_perturb * 1).to(tl.float16) + tl.sum(temp_12, axis=2)).to(tl.float16)
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        temp_13 = tl.load(C_exp_ptr + offset_14, mask=mask_12, other=0.0)
        ...
        temp_14 = tl.load(V_cache_ptr + offset_15, mask=mask_13, other=0.0)
        O = (tl.dot(temp_13, temp_14).to(tl.float16) + (O * 1).to(tl.float16)).to(tl.float16)
        offset_16 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None, None] * C_exp_perturb_stride0 + (tl.arange(0, 16))[None, :, None] * C_exp_perturb_stride1 + (p + tl.arange(0, BLOCK_P))[None, None, :] * C_exp_perturb_stride2
        mask_14 = (p_indices < P+M)[None, None, :]
        temp_15 = tl.load(C_exp_perturb_ptr + offset_16, mask=mask_14, other=0.0)
        offset_17 = (((n // BLOCK_N)+tl.arange(0, 1)))[:, None] * C_out_stride0 + (p + tl.arange(0, BLOCK_P))[None, :] * C_out_stride1
        mask_15 = (p_indices < P+M)[None, :]
        tl.store(C_out_ptr + offset_17, tl.sum((temp_15 / C_sum_perturb[:, :, None]).to(tl.float16), axis=1, dtype=tl.float16), mask=mask_15)
    ...
    tl.store(O2_ptr + offset_18, tl.reshape(temp_16, (M, D)), mask=mask_16)
