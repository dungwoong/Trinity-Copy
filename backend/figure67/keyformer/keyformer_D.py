...
def kernel_0(
    ...
    for p in range(0, 1024, BLOCK_P):
        ...
        C_sum = (tl.sum(tl.exp(temp_9.to(tl.float32)).to(tl.float16), axis=2, dtype=tl.float16) + (C_sum * 1).to(tl.float16)).to(tl.float16)
        C_sum_perturb = (tl.sum(tl.exp(C_perturb.to(tl.float32)).to(tl.float16), axis=2, dtype=tl.float16) + (C_sum_perturb * 1).to(tl.float16)).to(tl.float16)
    # Sequential loop p from 0 to 1024 with tile size BLOCK_P
    for p in range(0, 1024, BLOCK_P):
        ...
        tl.store(C_out_ptr + offset_13, tl.sum((temp_11 / C_sum_perturb[:, :, None]).to(tl.float16), axis=1, dtype=tl.float16), mask=mask_11)
    # Sequential loop p from 0 to 1024 with tile size BLOCK_P
    for p in range(0, 1024, BLOCK_P):
        ...
        O = ((O * 1).to(tl.float16) + tl.dot(temp_12, temp_13).to(tl.float16)).to(tl.float16)
