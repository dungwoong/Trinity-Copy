...
class KNGraph:
    ...
    def compile(self, async_=False, **kwargs):
        ...
        if result["max_smem_size"] > get_shared_memory_capacity(device_cc):
    ...
    def superoptimize(
        ...
        if backend == "cuda":
            ...
            # Limit concurrent compilations to reduce memory pressure
            if os.environ.get("MAX_CONCURRENT_COMPILATIONS", None) is not None:
                MAX_CONCURRENT_COMPILATIONS = int(os.environ["MAX_CONCURRENT_COMPILATIONS"])
            else:
                MAX_CONCURRENT_COMPILATIONS = 32  # Adjust this based on available memory

            # Collect all compilation tasks (store only metadata, not tensors)
            compilation_tasks = []
            for idx, g in enumerate(all_graphs):
                compilation_tasks.append((g, None, None))

            # Process compilations in batches
            for i in range(0, len(compilation_tasks), MAX_CONCURRENT_COMPILATIONS):
                batch = compilation_tasks[i:i + MAX_CONCURRENT_COMPILATIONS]
                handles = []

                for task in batch:
                    if len(task) == 3:
                        g, pipeline_stages, num_warp_groups = task
                    else:
                        g = task[0]
                        pipeline_stages = None
                        num_warp_groups = None

                    # Create input tensors only when needed
                    ...
                    if pipeline_stages is not None and num_warp_groups is not None:
            ...
            for idx, g in enumerate(all_graphs):
                ...
                for _ in range(warmup_iters):
                    g(inputs=input_tensors, benchmark=False)
