...
def rmsnorm_linear():
    """
    Constructs and superoptimizes a Mirage kernel graph for the RMSNorm
    followed by a linear projection.

    This function creates a kernel that applies RMSNorm
    to the input tensor, then projects the normalized output using a weight matrix.
    This is typically used as the prenorm step in attention mechanisms.

    Returns:
        KNGraph: A superoptimized Mirage kernel graph implementing RMSNorm and
                 linear projection.
    """
    ...
    x_norm = _graph.rms_norm(x, normalized_shape=(HIDDEN_DIM,))
    o = _graph.matmul(x_norm, w)

    _graph.mark_output(o)
    # Use mlp config for rmsnorm as standalone rmsnorm doesn't optimize well
...
def attention():
    ...
    # Shape: (NUM_HEADS, CONTEXT_LEN, HEAD_DIM)
    ...
    c_div = _graph.div(c_exp, c_sum)  # (NUM_HEADS, SEQ_LEN, CONTEXT_LEN + SEQ_LEN)
...
def run_cycle(
    ...
    # Prenorm - Apply RMSNorm once
    rmsnorm_linear_kernel = kernels[0]
    attention_kernel = kernels[1]

    # Apply RMSNorm to input once
    X_qkv: torch.Tensor = rmsnorm_linear_kernel(inputs=[x, w], stream=stream)[0]  # type: ignore
...
def main():
    parser = argparse.ArgumentParser(description="Prenorm Attention Benchmark")
    parser.add_argument(
    ...
    print("Running prenorm attention benchmark with:")
    ...
    rmsnorm_kernel = rmsnorm_linear()
    attention_kernel = attention()
    kernels = [rmsnorm_kernel, attention_kernel]
