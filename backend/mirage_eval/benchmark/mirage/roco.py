...
def run_cycle(
    ...
    attention_output, c_div = attention_kernel(
        inputs=[q, k_transposed, v_cache], stream=stream
    )  # type: ignore

    c_out1 = torch.sum(c_div, dim=1)
    c_out2 = torch.sum(torch.square(c_div), dim=1)
    ...
    return attention_output, c_out1, c_out2


def main():
    parser = argparse.ArgumentParser(description="RoCo Attention Benchmark")
    parser.add_argument(
    ...
    print("Running RoCo attention without PreNorm benchmark with:")
